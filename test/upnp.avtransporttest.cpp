...
public:
    ...
    : avtransport(std::make_unique<AVTransport::Client>(client))
    {
        ...
        // set a valid device
        EXPECT_CALL(client, getFile(g_serviceDescriptionUrl, _)).WillOnce(InvokeArgument<1>(200, testxmls::avtransportServiceDescription));
    ...
    void subscribe()
    {
        EXPECT_CALL(client, subscribeToService(g_subscriptionUrl, g_defaultTimeout, _))
            .WillOnce(InvokeArgument<2>(200, g_subscriptionId, g_defaultTimeout));
...
// {
//     Action2 expectedAction("Play", g_controlUrl, ServiceType::AVTransport);
